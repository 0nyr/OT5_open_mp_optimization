...
int main(int argc, char **argv)
{
	...
	half_float::half *A, *B, *C, cval, tmp, err, errsq;
	...
	A = (half_float::half *)MallocOrDie(Ndim*Pdim*sizeof(half_float::half));
	B = (half_float::half *)MallocOrDie(Pdim*Mdim*sizeof(half_float::half));
	C = (half_float::half *)MallocOrDie(Ndim*Mdim*sizeof(half_float::half));

	// Initialize matrices
	initArray(A, Ndim, Pdim, (half_float::half) AVAL);
	initArray(B, Pdim, Mdim, (half_float::half) BVAL);
	initArray(C, Ndim, Mdim, (half_float::half) 0.0);
    ...
    for (int i = 0; i<Ndim; i++) {
		...
		for (int j = 0; j<Mdim; j++) {
			tmp = 0.0;
			// reduction not working for custom type half
